...
listFragment.querySelector('.post-list__logout-btn').addEventListener('click',e=>{
  ...
  const listEl = listFragment.querySelector('.post-list__new-post-btn');
...
async function loginPage(){
  ...
  formEl.addEventListener('submit', async e => {
    const payload = { // 싣어서 보낼 객체 생성
      username: e.target.elements.username.value, // username: e.target.elements.username === fragment.querySelector('.login__username'); // target은 이벤트가 일어난 객체 e.target에는 form이 들어있음
